...
import "./interfaces/IBitenaMinting.sol";

/**
 * @title Bitena Minting
 * @notice This contract mints and redeems USDb in a single, atomic, trustless transaction
 */
contract BitenaMinting is
    IBitenaMinting,
    ...
    ReentrancyGuard
{
    ...
    bytes32 private constant ORDER_TYPE =
        keccak256(
            "Order(uint8 order_type,uint256 expiry,uint256 nonce,address benefactor,address beneficiary,address collateral_asset,uint256 collateral_amount,uint256 USDb_amount)"
        );
    ...
    address private constant NATIVE_TOKEN = 0x00;

    /// @notice EIP712 name
    bytes32 private constant EIP_712_NAME = keccak256("BitenaMinting");
    ...
    /// @notice USDb minted per block
    mapping(uint256 => uint256) public mintedPerBlock;
    /// @notice USDb redeemed per block
    ...
    /// @notice max minted USDb allowed per block
    uint256 public maxMintPerBlock;
    /// @notice max redeemed USDb allowed per block
    uint256 public maxRedeemPerBlock;

    /* --------------- MODIFIERS --------------- */

    /// @notice ensure that the already minted USDb in the actual block plus the amount to be minted is below the maxMintPerBlock var
    /// @param mintAmount The USDb amount to be minted
    ...
    /// @notice ensure that the already redeemed USDb in the actual block plus the amount to be redeemed is below the maxRedeemPerBlock var
    /// @param redeemAmount The USDb amount to be redeemed
    ...
    constructor(
        ...
        if (address(_USDb) == address(0)) revert InvalidUSDbAddress();
    ...
    function mint(
        ...
        belowMaxMintPerBlock(order.USDb_amount)
    {
        ...
        mintedPerBlock[block.number] += order.USDb_amount;
        _transferCollateral(
        ...
        USDb.mint(order.beneficiary, order.USDb_amount);
        emit Mint(
            msg.sender,
            order.benefactor,
            order.beneficiary,
    ...
    function redeem(
        ...
        belowMaxRedeemPerBlock(order.USDb_amount)
    {
        ...
        redeemedPerBlock[block.number] += order.USDb_amount;
        USDb.burnFrom(order.benefactor, order.USDb_amount);
    ...
    function verifyOrder(
        ...
        if (order.USDb_amount == 0) revert InvalidAmount();
